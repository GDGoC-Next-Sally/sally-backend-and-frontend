...
### 내부 서버 통신 (Internal Security)
- FastAPI AI 서버 등 내부 시스템 간의 통신은 `@Internal()` 데코레이터를 사용하여 JWT 검증을 우회합니다.
- 대신 헤더에 `x-internal-secret` 값을 실어 백엔드의 `INTERNAL_SECRET_KEY`와 일치하는지 검증합니다.
...
| `student_warning` | 선생님 (1:1) | 학생 이탈/포기 감지 | `{ dialog_id, student_id, reasons: string[], summary: string }` |
...
| `DELETE` | `/dialog/:dialogId` | ALL | 대화 내역 및 분석 데이터 삭제 |
...
### 5-4. Dev (개발자 도구)
Base URL: `/dev`
*이 모듈은 개발 환경에서만 활성화됩니다.*

| Method | Path | Role | 설명 |
|---|---|---|---|
| `POST` | `/seed` | PUBLIC | 테스트용 더미 데이터 세팅 및 JWT 토큰 발급 |
| `GET` | `/ui` | PUBLIC | 원클릭 채팅 및 소켓 모니터링 테스트 UI 제공 |
...
- [x] 원클릭 개발자용 채팅 및 소켓 모니터링 UI (`DevModule`)
- [x] 학생 경고 시스템 (`student_warning` 소켓 이벤트)
